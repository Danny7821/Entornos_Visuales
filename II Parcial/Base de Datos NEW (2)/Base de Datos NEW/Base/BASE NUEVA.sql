...
CREATE TABLE empleados (
   -- NomEmpleado int not null IDENTITY(1, 1),
	...
	CONSTRAINT FK_PUESTO foreign key (puesto) references puesto(puesto),
	CONSTRAINT FK_SEXO foreign key (sexo) references sexo(sexo)	
);
--select NomEmpleado,identidad,empNom,empApe,direccion,edad, sexo, puesto from empleados order by NomEmpleado  ASC

-- EMPLEADOS DE BULTO
INSERT INTO empleados(identidad,empNom,empApe,direccion,edad,sexo,puesto) 
...
CREATE TABLE planes (
	...
	CONSTRAINT FK_ID foreign key (identidad) references empleados(identidad)	
);
GO

--Valores para la tabla planes faltan los 9 que se meteran desde la interfaz
INSERT INTO planes(numeroTel,marca,Tplan,descripcion,identidad)
...
select * from empleados as EMP inner join planes as PLA on EMP.identidad = PLA.identidad;

-- Mostrar columnas especificas 
select EMP.identidad,EMP.empNom,EMP.empApe,EMP.puesto,PLA.numeroTel,PLA.marca,PLA.descripcion from empleados as EMP inner join planes as PLA on EMP.identidad = PLA.identidad 
...
as begin
select EMP.identidad as 'Identidad',EMP.empNom as 'Nombre Empleado',EMP.empApe as 'Apellido Empleado',EMP.puesto as 'Puesto Empleado',PLA.numeroTel as 'Telefono',PLA.marca as 'Marca Telefono',PLA.descripcion as 'Descripción del plan' from empleados as EMP 
inner join planes as PLA on EMP.identidad = PLA.identidad 
where (EMP.puesto ='Gerente' or EMP.puesto ='Jefe de Calidad' or EMP.puesto ='Jefe de TI Infraestructura' or EMP.puesto ='Jefe de TI Desarrollo' or EMP.puesto ='Jefe RRHH' or EMP.puesto ='Ejecutivo RRHH' or EMP.puesto ='Marketing' or EMP.puesto ='Medios' or EMP.puesto ='Soporte TI')
end
